...
fn compile_report(report_dir: Option<std::path::PathBuf>) -> Result<(), Box<dyn Error>> {
    ...
    let mut report_title = "Pentest Report";
    let mut prepared_for = "Example Data";
    let mut prepared_by = "Example Data";
...
fn new_report(report_dir: Option<std::path::PathBuf>) -> Result<(), Box<dyn Error>> {
    // Ensure user provided the report path
    let report_path = report_dir.unwrap_or_else(|| {
        eprintln!("ERROR: Report path not provided");
        exit(1);
    ...
    */
    ...
    f_metadata.write_all(
        b"title:Example Pentest Report
